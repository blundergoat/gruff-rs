...
Error-handling and concurrency rules are deliberately syntactic. Production panic and placeholder findings skip test functions and helpers inside `tests` modules; public API unwrap findings add API-context signal on top of the broader unwrap/expect rule. Async/concurrency findings use medium confidence unless the scanner has only a narrow source pattern such as an async function calling `std::thread::sleep`, a lock binding that appears to cross `.await`, or an unbounded channel constructor. The lock-across-await rule requires a bound, zero-argument acquisition: `.lock()` remains an explicit lock-named heuristic, while ambiguous `.read()` and `.write()` calls require the file to tie that receiver to a `Mutex` or `RwLock` type or constructor. That evidence comes from the acquiring function first and then from the rest of the file, so a guard taken from a lock-typed struct field such as `self.state.write().await` reports while an I/O or domain `read`/`write` on an unrelated receiver stays silent. Guard-preserving `.await`, `.unwrap()`, `.expect(...)`, and `?` suffixes remain covered; calls with arguments, immediate value extraction, explicit drops, and guards scoped out before the later await stay silent. These checks complement Clippy by making the patterns visible in Gruff reports and scoring; they do not resolve type aliases or cross-file types and do not claim runtime deadlock certainty or type-aware error taxonomy.
...
Step keys are read in every spelling GitHub accepts, including quoted keys such
as `- "run":` and block headers carrying an indentation or chomping indicator
(`|2`, `>-`, `|2-`). Pinning covers a step `uses:`, a job's reusable workflow
`uses:`, and container images: a `docker://` reference is exempt only when it
names an immutable `@sha256:` digest, because a tag such as `docker://alpine:latest`
still runs whatever the registry serves next. The remote-shell rule requires the
downloaded payload to reach the interpreter — a pipe always delivers it, while a
`;` or `||` command counts only when the interpreter is invoked bare or reads a
path the downloader wrote — so running a checked-in script after an unrelated
...
permission contract. Pull-request gating reads the top-level `on` mapping only,
so a `with:` input or matrix key named `pull_request` is not a trigger, while a
quoted `"on":` key or quoted event name still is; secret expressions match with
or without interior whitespace. The permissions rule reports workflow-level
scoped writes and `permissions: write-all` at any level outside a step, where
that key is an action input rather than a grant; a bounded job-scoped mapping
such as release-only `contents: write` stays silent.

`sensitive-data.high-entropy-string` reports at warning with medium confidence and reads two named bars under `rules.sensitive-data.high-entropy-string.thresholds`: `minLength` (default 32) and `entropy` (default 4.2, in bits per character). A literal must also hold at least one letter and one digit, the floor FAMILY-CONTRACT section 12 sets, because a run of one character class clears the entropy bar by construction and a digit-free mix of cases is an identifier. Text inside a PEM block whose label names no private key (a certificate, public key, certificate request, PKCS7 bundle or CRL) never reports, because it is public by construction; a private key's block is still scanned. Sensitive-data rules report likely secret material directly from text. Identity-independent findings serialize only zero-payload detector markers such as `[redacted]`, `[redacted:private-key]`, or `[redacted:connection-string:https]`; source characters and the historic partial aliases are not report metadata. The 0.5 key `allowlists.secretPreviews` is removed and a configuration carrying it is refused: section 5 makes every marker unconditional, so the key authorised nothing. `sensitive-data.database-url-password` is scoped to database and message-bus URL schemes, while `sensitive-data.url-embedded-credentials` covers generic HTTP(S) `user:password@host` URLs and skips reserved documentation hosts and obvious placeholder credentials. `sensitive-data.high-entropy-string` keeps real base64/base64url/JWT/PAT-like secret shapes reportable while skipping known non-secret structures: package integrity literals including `sha1-`, exact standard and URL-safe base64 alphabets, separator-bearing word slugs, and provider/model identifiers such as `provider/Family/Model-Size-Variant`. It deliberately does not infer that arbitrary checksum or signature fields are safe from value shape alone. Provider-prefixed token coverage stays under `sensitive-data.api-key-pattern` unless a separate rule ID adds distinct user-facing value; GCP service-account key material and PHI-style identifiers have dedicated IDs because their remediation and triage differ from generic tokens.

As of 0.6.0, `sensitive-data.pii-test-fixture` also carries a zero-payload marker: its message reads ``Realistic email value `[redacted:email]` found in a fixture/sample file``, and the matched characters and their count no longer appear anywhere. This was the deferral 0.5.0 recorded, and the coordinated family JSON break is the release that lands it. Because the line-scoped `stableIdentity` hashes the message, masking it gives every occurrence of one category in one file the same identity; hook new-only matching already resolves that by counting identical identities, so the behaviour is the same one every constant-message rule has always had. Identity values therefore change once in 0.6.0 and users re-baseline once.
