...
impl DocCommentText {
    ...
    pub(crate) fn mentions_identifier(&self, name: &str) -> bool {
        let lower = self.0.to_ascii_lowercase();
        let needle = name.to_ascii_lowercase();
        let pattern = needle.as_str();
        let bytes = lower.as_bytes();
        let pattern_bytes = pattern.as_bytes();
        let mut index = 0usize;
        while let Some(found) = lower[index..].find(pattern) {
            let absolute = index + found;
            let before_ok = absolute == 0 || {
                let byte = bytes[absolute - 1];
                !byte.is_ascii_alphanumeric() && byte != b'_'
            };
            let after_pos = absolute + pattern_bytes.len();
            ...
            index = absolute + pattern_bytes.len();
        }
        false
    }

    pub(crate) fn mentions_returns(&self) -> bool {
