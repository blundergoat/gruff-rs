...
Treat this repository root as the selected target workspace. Parent workspaces, npm package internals, and other agent surfaces are context only unless the user explicitly widens scope.

## Autonomy Tiers

**Always:** Read relevant files before edits, keep changes scoped, use `apply_patch` for manual file edits, run the smallest real verification command that covers the change, and update `.goat-flow/` memory when verification changes the approach.

**Ask First:** Before changing report schemas, rule IDs, fingerprints, baseline/history behavior, dashboard cwd handling, `fixtures/`, `.codex/`, `.agents/skills/`, `.goat-flow/config.yaml`, hooks, or 3+ docs/scripts, state the boundary, files read, matching footgun/lesson checked, local instruction checked, and rollback command.

**Never:** Do not edit secrets, push, commit, run destructive git commands, overwrite existing instruction content, delete fixtures as "bad code", or change non-Codex peer instruction files without explicit user direction.
