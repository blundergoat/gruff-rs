...
4. Update every calibration positive case for the rule. Grep `"<rule-id>"` in `src/tests/calibration/` and inspect the fixture body the positive closure writes.
...
Same ripple applies when deleting a rule: registry entry in the matching shard under `src/rules/`, scanner implementation in `src/built_in_rules/*.rs`, calibration case in the matching shard under `src/tests/calibration/`, calibration matrix harness if it special-cases the rule, default config entry in `.gruff-rs.yaml`, and any test asserting on the rule ID (search: `rule_id ==` and `assert_has_rule`).
