...
`gruff-rs` is a Rust binary composed of focused modules. The binary has nine user-facing modes: `analyse` walks source trees and emits findings, `hook` emits the `gruff.hook.v1` coding-agent hook contract JSON, `report` renders analysis output to a file or stdout, `summary` prints a compact digest (scan card + per-pillar / top-rules / top-files), `list-rules` prints registry metadata, `dashboard` starts a small local HTTP server for browser-driven scans, `check-ignore` reports whether gruff would ignore each path and why using the same config and ignore engine as `analyse` (it runs no analysis), `init` writes a default `.gruff-rs.yaml` config derived from the built-in rule registry, and `completion` emits shell completion scripts.

Module layout: `src/main.rs` is the entry point and command dispatch (timing each scan with `Instant::now()` for the text/summary scan card); `src/cli/` owns Clap argument structs and global option/output-writer/run-outcome types; `src/analysis.rs` is the `run_analysis` orchestrator; `src/discovery.rs` handles Git-ignore-aware source walking; `src/parser/` and `src/source.rs` produce parsed sources; `src/project/` builds `ProjectContext` and the identifier-count index; `src/analyse_project/` runs project-pillar rules (architecture, dead-code, dependencies); `src/built_in_rules/` runs per-source rules; `src/custom_rules.rs` evaluates config-defined regex rules; `src/baseline.rs`, `src/diff.rs`, `src/config.rs`, `src/config_loader/` cover suppression/diff/config layering; `src/report.rs` defines the public report types and schema strings; `src/report_identity.rs` owns scope inference and stable-identity hashing; `src/hook.rs` renders the `gruff.hook.v1` contract surface; `src/scoring.rs` computes composite/per-pillar scores; `src/summary.rs` renders the `summary` digest; `src/render/` holds the text/markdown/github/hotspot/sarif renderers; `src/html_report/` renders the HTML inspection report; `src/dashboard.rs` is the local HTTP server. Rule metadata and the built-in rule registry live in `src/rules/` (definitions split across `structure_docs_reliability_definitions.rs`, `idiom_security_size_test_definitions.rs`, and `waste_definitions.rs`, then re-exported from `mod.rs`); built-in rule dispatch is behind the `src/built_in_rules/` module boundary. Supporting shell entrypoints live in `scripts/preflight-checks.sh` and `scripts/start-dev.sh`; intentionally noisy analyzer inputs live in `fixtures/` and `tests/fixtures/`.

## Request Flow

For CLI analysis, `main` parses Clap commands, `run_analysis` loads config, `discover_sources` walks input paths (defaulting empty paths to the current directory in both `discovery::resolve_input_paths` and `RequestedScope::from_options`), reads each file into a parsed source record, builds an internal `ProjectContext`, dispatches project rules, then dispatches text, Rust, and config-defined regex custom rules through `analyse_source`. Exact baselines apply before report-level exclusions, and optional patch-input diff filtering runs last, so accepted historical findings stay suppressed before audited exclusions and changed-line narrowing. `render_report_with_scope` selects text, JSON, SARIF, HTML, Markdown, GitHub annotation, or hotspot output, threading a renderer-only `RequestedScope` (requested paths plus diff label) into HTML rendering without serialising it onto `AnalysisReport`. The text and summary renderers also receive an `Option<u128>` scan-duration measured at the CLI handler so the scan card can show wall-clock time without touching the report schema. `summary::render` reuses the same `AnalysisReport` and emits either a text scan card + per-pillar/top-rules/top-files digest or `gruff.summary.v2` JSON. `list-rules` loads the same config surface, renders sorted built-in registry definitions followed by sorted `custom.<slug>` rules as text or JSON, and `list-rules --selector` previews the deterministic rule-id set matched by an exact id, dotted prefix, or public pillar selector.

The HTML inspection report lives in `src/html_report/` (entry point `mod.rs`, sections in `sections.rs`, CSS in `styles.rs`). It builds a renderer-internal `ReportView` view-model that derives pillar grade letters, per-pillar severity counts, top-offender grade pills, and a bucketed cyclomatic-complexity distribution from finding metadata. Visual identity (italic-serif `gruff.rs` wordmark, L-bracket "paper" container, rotated grade stamp, pillar grid, complexity histogram, severity-tagged finding rows, forge-orange accent) is shared by `analyse --format html` and the dashboard iframe body.
...
The analyzer reads user-selected files and does not execute analyzed source. Patch-input diff mode reads unified diff text as data and does not run Git or external diff drivers, following `.goat-flow/learning-loop/decisions/ADR-009-suppression-baseline-and-diff-layering.md`. The Git-backed `--diff`/`--since` modes are gated behind `--diff-git-unsafe` — a hard error without that opt-in, and the flag is hidden from `--help`. They are the documented opt-in exception to the no-execute posture (ADR-008), recorded with their residual risk in ADR-019; the shared Git subprocess builder uses argv only, disables pagers and hooks, removes `GIT_EXTERNAL_DIFF`, and neutralizes global/system config and attributes. The Git-free `--diff-patch`/`--changed-ranges` modes need no opt-in and are the default-safe path. Fixture files deliberately contain command execution, secret-looking strings, parser edge cases, and noisy rule examples so the scanner can prove those rules fire; those fixture strings are test inputs, not runtime credentials.
...
serialized into `gruff.analysis.v2`.

Project rules run against `ProjectContext`. Text rules run for every supported file. Rust AST rules run only when parsing succeeds; parse failures emit a `parse-error` diagnostic while still allowing text-only checks such as sensitive-data detection. Rust function-scope rules include complexity, naming, test-quality, error-handling, async/concurrency, loop-scoped performance, and deterministic token metrics. Regex custom rules are loaded from top-level `custom_rules`, compile during config loading, and run after built-ins with explicit `text`, `rust-code`, or `comments` scopes plus optional include/exclude path filters. Findings are sorted and deduplicated by fingerprint before rendering. Baseline generation and history recording write JSON side files whose names are ignored by `.gitignore`; absence of those files in a clean checkout is normal. Report-level `exclude` entries are post-analysis suppressions with required reasons and counts; they filter rendered findings without changing discovery, rule ids, fingerprints, or exact baseline matching. `--diff-patch` filtering is report post-processing after exclusions: it preserves analysed file counts and finding identity, recomputes report summary/score after filtering, and records an informational `patch-filter` diagnostic with kept and suppressed counts. When a baseline or diff-patch comparison context is active, `apply_baseline` and `apply_diff_patch_filter` populate `AnalysisReport.per_rule_deltas` with `{ruleId, introduced, removed, net}` entries (ADR-014); text, Markdown, and `summary` reporters render `Top 5 improved` and `Top 5 regressed` blocks before the composite-score line, and the JSON field is omitted entirely on full-tree runs so the schema stays byte-identical for non-comparison consumers. Baseline application also records tri-state counts (`new`/`unchanged`/`absent`) on the report additively. After exclusions, an optional `gate:` block (ADR-003) evaluates finding counts — per severity and/or a total cap — and records a non-fatal `gate` diagnostic; `gate.scope` (`new`/`all`, default unset) selects the post-baseline (new-only) or pre-baseline (all) count set, and `--fail-on-new` is the CLI alias for `scope: new`. `RunOutcome::classify` consults the gate before the legacy `--fail-on` flag, and `scope: new` with no applied baseline is a fatal config error (exit 2).
...
- `src/report.rs` (`gruff.analysis.v2`), `src/summary.rs` (`gruff.summary.v2`), `src/render/hotspot.rs` (`gruff.hotspot.v1`), and `src/render/sarif.rs` (SARIF 2.1.0) contain the public schema strings; changing them is a compatibility decision.
...
- Advanced metric rules use deterministic token counts and calibrated single thresholds for advisory findings. They complement, but do not replace, the existing `score` object in `gruff.analysis.v2`.
