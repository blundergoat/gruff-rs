...
- `github-actions/` contains retained GitHub metadata negatives with immutable
  source, license, and content-hash provenance in each fixture header.
...
The retained PyPA cibuildwheel composite action exercises legitimate block
`run:` steps and a full-SHA third-party dependency. It must stay silent for the
event-interpolation, remote-shell, and unpinned-action rules enabled on explicit
action metadata.
