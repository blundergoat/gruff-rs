...
- **Fixed: a raw identifier no longer aborts a baseline, and its identity changes once** - A finding on `r#match` carried the `#` of the raw prefix, which reads as the ordinal separator, so `--generate-baseline` and every baseline-applying run exited 2. The identity now names a raw identifier by its plain spelling (`r#match` is `match`, `Parser::r#type` is `Parser::type`), so such a finding is baselined like any other. A baseline written before this change holds no row for it, so regenerate once.
- **Fixed: `error-handling.public-unwrap` and `error-handling.unimplemented-placeholder` skip test-infrastructure paths** - A `pub fn` in an integration-test support module such as `tests/support/` has no public API contract, and `error-handling.production-panic` already skipped those paths; the 11-repository Rust corpus held 128 such `public-unwrap` findings. `unimplemented-placeholder` also stops reading a macro named in a comment or quoted into a `quote!` stream, and its `metadata.macros` now lists only the macros the fn actually calls.
- **Fixed: seven Rust rules stop reporting correct code** - Over the 11-repository Rust corpus, findings fall from 31,710 to 29,338: every removal is in a repaired rule, and one block moves from `security.unsafe-block` to `docs.weak-safety-rationale`. `docs.missing-return-doc` reads a `-> Self` constructor, a summary opening `Return`, `Get` or `Create` followed by the value it names, and a getter taking only `&self` whose descriptive summary names the fn's final word after its opening word (as `/// Logging filter level.` on `fn log_level`). `error-handling.unimplemented-placeholder` ignores the macro name in comments and inside `quote!` bodies. `test-quality.should-panic-without-expected` accepts `#[should_panic = "msg"]` and skips a test whose own `cfg` leaves it out of every test build. `security.unsafe-block` finds a `SAFETY` rationale across a line that leads into the block, as the first comment inside it, and under a bare `SAFETY` line or a `# Safety` line, and `docs.weak-safety-rationale` now reports a rationale whose first word is `todo`, `fixme`, `xxx` or `hack`. `docs.commented-out-code` reports only a comment that parses as Rust, outside a fenced example or a clippy UI test file; a snippet over 16 KiB or nested too deeply to parse safely is left alone, so comment text can never overflow the parser. `dead-code.unused-private-function` and `dead-code.unused-private-item-candidate` treat as reached a fn whose attribute's last segment is `test` or `bench` or ends in `_test` or `_bench`, a fn named by serde's `default`, `deserialize_with`, `serialize_with`, `with` or `skip_serializing_if` (also through `cfg_attr`), and an empty-body `where` assertion over concrete types.
- **BREAKING: `sensitive-data.high-entropy-string` reports at warning, and its two bars are configurable** - The rule reported at `error`; it now defaults to `warning` with medium confidence, the family contract gruff-go, gruff-php, gruff-py and gruff-ts ship. `rules.sensitive-data.high-entropy-string.thresholds.minLength` (a whole number from 1 to 65535, default 32) and `.entropy` (zero or more, default 4.2) are read from `.gruff-rs.yaml` instead of being hard-coded, validated when the config loads, and published by `list-rules` and seeded by `init`. A lowered `minLength` now reaches shorter literals; before, the pattern itself required 32 characters. A standalone `severity` override still works, and every other rule still refuses a `thresholds:` map.
- **BREAKING: `list-rules --format json` is an object carrying the rules under `rules`, and thresholds are a named knob map** - The catalogue was a bare array; it is now `{"rules": [...]}`, the shape the other four ports already publish. A rule's scalar `threshold` moves to `thresholds`, as `{"maxLines": 1000}` where gruff-go already names the knob and `{"threshold": 25}` where no port does, printed as an integer when the default is one; a rule with no threshold publishes neither key. `list-rules --selector` output and `list-rules <id> --format json` are unchanged. SARIF rule properties and `.gruff-rs.yaml` keys are unchanged.
...
- **`--fail-on` help names the key that gates it.** `analyse --help` and `report --help` said the flag falls back to `minimumSeverity.analyse:` and `minimumSeverity.report:` in `.gruff-rs.yaml`, a per-command map the loader now refuses with exit `2`; they name `failOn.analyse:` and `failOn.report:`. The `init --force` diagnostic for an unparseable config now reads `failOn preservation`, which is the block it actually preserves.
