...
- **Process-command findings require standard-library provenance.** `security.process-command` now accepts bare `Command` only when a root `use` resolves it to `std::process::Command`, recognises `use std::process` with `process::Command`, and ignores other crates' same-named builders, unimported bare names, and comment-only examples. Clap's measured findings fall from 1,265 to 6, so existing baselines can shrink sharply in projects that use another crate's `Command` type.
...
- **SAFETY rationales follow Rust comment conventions.** `security.unsafe-block` now matches `SAFETY:`, `Safety:`, and `safety:`, and joins a bounded multiline comment prelude without crossing executable code. Tokio contains 303 documented and 499 undocumented unsafe sites; findings fall from 704 to 499, so affected baselines can remove the 205 newly recognised rationales.
...
- **The `++` scan bypass stays closed on the Bash 4+ path.** goat-flow 1.15.1 reopened it: its optimized diff walk skipped any candidate line starting with `+++` regardless of position, so an added source line beginning with `++` — rendered `+++...` by `--unified=0` — was dropped and a credential on it ended the turn with exit 0, while the Bash 3 fallback still blocked. Only a hunk header is skipped there now, because a real destination header is already consumed by the position-guarded branch above it.
